...
jQuery(document).on('DOMNodeInserted','.activate-now', function () {
    ...
    if (activateButton.length) {
        ...
        if (typeof url !== 'undefined') {
            ...
            jQuery.ajax({
                beforeSend: function () {
                    jQuery(activateButton).replaceWith('<a class="button updating-message">' + Startkit_Plugin_Helper.activating + '...</a>');
                },
                async: true,
                type: 'GET',
                url: url,
...
jQuery(document).ready(function ($) {
    $('body').on('click', ' .startkit-install-plugin ', function () {
    ...
    $('.activate-now').on('click', function (e) {
        ...
        if ($(activateButton).length) {
            ...
            if (typeof url !== 'undefined') {
                ...
                $.ajax({
                    beforeSend: function () {
                        $(activateButton).replaceWith('<a class="button updating-message">' + Startkit_Plugin_Helper.activating + '...</a>');
                    },
                    async: true,
                    type: 'GET',
                    url: url,
